...
namespace _ComponentCustomProperties {
  ...
  const { gtmEnabled, gtmId }: typeof import('./src/composables/gtm')
  const { injectHead, useHead, useHeadSafe, useSeoMeta, useServerHead, useServerHeadSafe, useServerSeoMeta }: typeof import('@unhead/vue')
  const { isDark, setDarkMode, toggleDark }: typeof import('./src/composables/dark')
  const { onBeforeRouteLeave, onBeforeRouteUpdate, useRoute, useRouter }: typeof import('vue-router')
  const { useAuthStore }: typeof import('./src/store/auth')
  const { useI18n }: typeof import('vue-i18n')
  const { usePageTransition, useTransitionIn, useTransitionOut }: typeof import('./src/composables/transitionEvents')
  const { useSiteStore }: typeof import('./src/store/site')
  const { useUserStore }: typeof import('./src/store/user')
}
declare module 'vue' {
